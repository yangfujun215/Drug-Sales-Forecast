...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        lstm_out = lstm_out + attn_output  # 残差连接
...
# 定义模型
model = Model(1,64,1,num_layers=10,num_heads=2).to(device)
    ...
    if (epoch + 1) % 1 == 0: # if (epoch + 1) % 10 == 0:
        ...
        with torch.no_grad():
            for test_batch in test_loader:
                ...
                test_gru_outputs = model(test_inputs)
...
# torch.save(model, os.path.join(save_directory, 'model_complete_400_log_test.pth'))
